...
- (IBAction)deleteCellAction:(UIButton *)sender {
    // 千万不要一边遍历，一边删除----没删除一个数组元素，其他元素的索引可能会发生变化
    NSMutableArray *deleteArray = [NSMutableArray array];
    for (NSIndexPath *index in self.tableView.indexPathsForSelectedRows) {
        // 存在数组里，后期一次性删除数据源
        [deleteArray addObject:self.wineArray[index.row]];
    }
...
- (IBAction)multipleDeleteCellAction:(UIButton *)sender {
    self.deleteBtn.hidden = self.tableView.isEditing ? YES : NO;
    // 没有动画的进入编辑模式
